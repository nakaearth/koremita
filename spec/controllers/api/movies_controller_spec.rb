...
module Api
  describe MoviesController, type: :controller do
    ...
    let!(:movies) { create_list(:test_movie, 3,  user: current_user, created_at: exec_time) }
    let!(:movies2) { create_list(:test_movie, 5,  user: users[0], created_at: exec_time) }
    let!(:movies3) { create_list(:test_movie, 8,  user: users[1], created_at: exec_time) }
    let!(:movies4) { create_list(:test_movie, 10,  user: users[2], created_at: exec_time) }
    ...
    describe '友人の映画一覧を返す' do
      context 'ログインしている場合' do
        ...
        it '友人をランダムに選び映画の一覧を返す' do
          # expect(assigns[:movies].size).to eq(23)
        end
      end
    end
  end
